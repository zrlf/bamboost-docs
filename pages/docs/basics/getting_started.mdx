...
```python filename="Creating a database"
...
Internally, a sqlite database is maintained at `~/.local/share/bamboost/bamboost.db` to keep track of known bamboost
...
```python filename="Creating a new entry in a database"
parameters = {
    "boat": {
        "E": 1,
        "nu": 0.3,
...
db.df
```
|   | id | notes | status | time_stamp | boat.E | boat.nu | material_model | penalty_constant | processors | wall.E | wall.nu |
|---|---|---|---|---|---|---|---|---|---|---|---|
| 0 | e25559b8 | | Initiated | 2024-07-21 17:52:43 | 1 | 0.3 | Linear elastic | 100 | 1 | 20 | 0.2 |
...
In the TUI, you can review all your databases in a single place. [Check out the page for the TUI](/docs/basics/TUI)
...
```python filename='Add field data'
sim.add_field(
    name='name_of_field',
    vector=np.array([...]),
...
```python filename='Add global data'
...
```python filename='Adding a subgroup'
new_group = sim.userdata.require_group(name)
```

```python filename='Adding an array or a single value'
