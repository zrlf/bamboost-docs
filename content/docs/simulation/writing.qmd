...
Collection(uid="315628DE80").add("mesh_reference", override=True);
```
...
# 2) Convert an existing read-only simulation to a writer
...
::: {.callout-tip}
`SimulationWriter` also supports Python's context manager protocol, which automatically
sets the simulation status to `STARTED` on entry and `FINISHED` (or `FAILED`) on exit.
This is especially useful in real execution scripts to ensure status is always up to date.
:::
...
```{python}
sim.parameters["mesh_cells"] = 256
sim.parameters.update({"dt": 5e-3, "nu": 1e-3})
sim.metadata["git_hash"] = "9a58e4c"
sim.links["baseline"] = "315628DE80:mesh_reference"
```
...
for step_id, t in enumerate(times):
    ...
    step = sim.data.require_step(value=float(t), step=step_id)
