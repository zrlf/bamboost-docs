...
coll = Collection(uid="C8C4A62EB9")
# remove all existing simulations for demonstration purposes
coll.delete(coll.all_simulation_names())

# fill the collection with some simulations
for i in range(6):
  params = {
    "E": 20 + i,
    "nu": 0.1 + i * 0.05,
  ...
  coll.add(parameters=params, tags=["demo"])

tags = coll[0].tags
tags.add("epic")
coll[0].edit().tags = tags
...
- **create_if_not_exist**: Default is `True`. When `True`, a new collection is created at the given path if it does not already exist. When `False`, an error is raised if the path does not correspond to an existing collection.
...
sim = coll.add("my_new_one", parameters=params)
```

The remaining arguments of `add` are:

- **duplicate_action**: Action to take if a simulation with the same parameters already exists. Options are: "ignore" (create anyway), "replace" (delete existing and create new), "skip" (return existing simulation), "raise" (default, raise DuplicateSimulationError).
...
Now, if we print the collection again, we see the new simulation:
```{python}
coll.df
```
...
Use the `duplicate_action` argument of `add` to specify how to handle duplicates. You have the following options:

| Action | Description |
| --- | --- |
| `ignore` | Create the new simulation anyway, even if a duplicate exists. |
| `replace` | Delete the existing simulations that match and create the new one. |
| `skip` | Don't create the new simulation, return the existing one instead. |
| `raise` (default) | Raise a `DuplicateSimulationError` if a duplicate is found. |
